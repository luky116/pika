...
PikaClientProcessor::PikaClientProcessor(size_t worker_num, size_t max_queue_size, const std::string& name_prefix) {
  ...
  for (size_t i = 0; i < worker_num; ++i) {
    ...
    bg_threads_.back()->set_thread_name(name_prefix + "BgThread_" + std::to_string(i));
    LOG(INFO) << "init PikaClientProcessor, i= " << i << ", is_runnint " << bg_threads_.back()->is_running() << ", name " << bg_threads_.back()->thread_name();
  }
}
